...
enum QObject { Empty, Deck, Miss, HitDeck };
// множество состояний корабля
enum Q_PROPERTY { Destroyed, ShipHit, Safe };

// класс - клетка игрового поля
class Unit {
private:
    int _x, _y;         // координаты клетки
    QObject _state;   // состояние клетки
public:
    Unit(int x = 0, int y = 0, QObject state = Empty)
    {
        _x = x;
        _y = y;
        _state = state;
...
{
private:
    Unit* _cells;  // клетки корабля
    Q_PROPERTY _state;   // состояние корабля
    int _size;
public:
    Ship()
    {
...
private:
    ...
    Unit _cells[_size][_size]; // клетки игрового поля
...
public:
    ...
    void SetState(int x, int y, QObject state)
    {
